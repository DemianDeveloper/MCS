...
# 1. робот ищет монету с наибольщим движением на binance 
# 2. проверяет продолжает ли она движение в short
# 3. заходит в short соблюдая TP/SL
...
TELEGRAM_TOKEN = '6732784773:AAEXp9LFuu4WY7tkNTW89fHf5ECDHsUUPqU'
TELEGRAM_CHANNEL = '@klimov_vremenno_ydalit'
TF = '15m'
TP = 0.03
SL = 0.01
DEPOSIT = 10
...
def get_trade_volume():
    volume = round(DEPOSIT/get_symbol_price(symbol))
    # print(type(volume))
...
while True: 
    if close[-2] < close[-3] and open_position == False:
        print("The coin goes down")
        send_message("The coin goes down")
        ...
        send_message("Was open Stop order")
        time.sleep(2)
        open_take_profit_order(symbol, take_profit_price, volume)
        send_message("Was open Take Profit order")

    else:
        print("The coin not goes down")
        send_message("The coin not goes down")